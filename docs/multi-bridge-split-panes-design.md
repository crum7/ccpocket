...
## 6. 決定事項（2026-06-05・UX 優先で確定）

iTerm2/tmux ライクな操作感を軸に確定。

1. **アイドル接続は保持する。** 繋いだ bridge は全部生かし続け（各々が独立して自動再接続）、ユーザーが明示的に切断/削除した時だけ落とす。現状の単一接続の挙動（繋ぎっぱなし + 自動再接続、意図的切断のみ）を踏襲。
   - 理由: ペイン切替・別マシンのセッション参照が**待ち時間ゼロ**で済む。再接続ラグが UX を一番損なう。

2. **1 タブ = 1 分割ツリー。** macOS タブそれぞれが独立した分割レイアウト（ワークスペース）を持つ。
   - 理由: iTerm2 と同じメンタルモデル。タブで作業セットを切り替え、各タブ内で自由に分割。

3. **同一セッションの複数ペイン同時表示は許可する**（ブロックしない）。各ペインは同じ接続のセッションストリームを購読するので自動的に同期表示される。
   - 理由: 「片方で diff、片方でチャット」等の需要に応えられる。禁止する方が摩擦。

4. **永続化はワークスペース全体（アプリ全体で1セット）。** 全タブ + 各タブの分割ツリー + 各ペインの `SessionRef` を保存し、起動時に丸ごと復元。接続先/セッションが消えている場合はそのペインを空（ピッカー）にフォールバック。
   - 理由: 「閉じて開いたら前回のまま」が最良の UX。マシン別/プロジェクト別の出し分けは過剰。

5. **キーバインドは iTerm2 ライクな ⌘ ショートカット**（tmux の prefix 方式は採らない）。
   - `⌘D` = 縦分割（右にペイン追加）/ `⌘⇧D` = 横分割（下にペイン追加）
   - `⌘W` = フォーカス中ペインを閉じる / `⌘⌥←→↑↓` = ペイン間フォーカス移動
   - `⌘T` = 新規タブ
   - 理由: macOS GUI で prefix 方式は異質。iTerm2 利用者にそのまま馴染む。

> これらは Phase 2/3 着手時の前提とする。実装中に判明した制約があれば本節に追記する。
